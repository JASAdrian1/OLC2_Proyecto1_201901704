...
class AccesoArreglo(Expresion):
    ...
    def getValor(self, contralador, ts):
        ...
        if arreglo is not None:
            ...
            if isinstance(arreglo.estructuraArr, InicializacionVector):    #Se verifica si es un vector o un arreglo para obtener el limite de los indices de estos
                valoresLimites = []
                valoresLimites.append(len(arreglo.estructuraArr.expresion))     #Si es vector se almacena de esta forma
                print(arreglo.estructuraArr.expresion)
            else:                                                               #Si es un arreglo se almacena de esta otra
            ...
            if seExcedeLimit == False:
                ...
                if isinstance(arreglo.estructuraArr, InicializacionVector) is False:    #Si es un arreglo se accede al arreglo de esta forma
                    for dimension in valoresAcceso:
                        valorRetornar = valorRetornar[dimension]
                else:                                                                   #En cambio si es vector se accede de esta otra forma
                    valorRetornar = valorRetornar.expresion[valoresAcceso[0]]
                #print(valorRetornar,"-.-.-.-.-.-.")
